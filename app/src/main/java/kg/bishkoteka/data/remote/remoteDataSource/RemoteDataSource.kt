...
//    suspend fun createEvent(organizationId: Int, eventDto: EventCreateRequest) = getResult { eventsApiService.createEvent(organizationId, eventDto) }
    suspend fun createEvent(organizationId: Int, map: MutableMap<String, RequestBody>) = getResult { eventsApiService.createEvent(organizationId, map) }
    suspend fun getEventById(eventId: Int) = getResult { eventsApiService.getEventById(eventId) }
    suspend fun addComment(eventId: Int, commentRequest: CommentRequest) = getResult { eventsApiService.addComment(eventId, commentRequest) }
